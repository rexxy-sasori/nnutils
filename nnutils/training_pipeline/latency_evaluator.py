...
def eval_sparsednn_c_api(model_state, inputs_dict, target_device, num_trials):
    ...
    for idx, (target_layer_name, input_detail) in enumerate(inputs_dict.items()):
        ...
        # code generation
        ret_code = os.system(
            f'python code_gen_cpu.py --A_dim {M} --B_dim {K} --C_dim {N} --AT 6 --CT 2 --B_blocks 1 --C_blocks 1 --Gy 1 --infile matrix_transposed.npy --outfile testing.cpp --outfile_asm test1.s --x86 --no_relu --infile_bias bias.npy --fuse')
        if ret_code != 0:
            raise Exception(
        ...
        # icc
        ret_code = os.system(
            f"icc -fopenmp -I . -qmkl -O3 -march=native -D AT=6 -D CT=2 -D C_Blocks=1 -DA_dim={M} -DINFILE=matrix_transposed.npy -D B_dim={K} -D C_dim={N} -D C_blocks=1 -D X86=1 -D MULTI=0 driver_cpu.cpp -lcnpy -o test -std=c++17")
        if ret_code != 0:
            raise Exception(
